...
public class hw4_b {
	...
	public static void Midpoint(double start_point,double end_point){
		...
		for(int m=1;;m++){
		    ...
		    for(int j=0;j<=n/2;j++){
		    	temp+=Math.exp(start_point+(2*j+1)*h);
		    }	    
		    //must add abs
	...
	public static void Trapezoidal(double start_point,double end_point){
		...
		for(int m=1;;m++){
		    ...
		    for(int j=1;j<=n-1;j++){
		    	temp+=Math.exp(start_point+(j*h));
		    }	    
		    //System.out.println(Math.abs((((2*temp)+Math.esp(start_point)+Math.esp(end_point))*h/2)));
		    if(Math.abs((((2*temp)+Math.exp(start_point)+Math.exp(end_point))*h/2)-anser_value)<0.000001 && m!=1){
		    	anser_m=m;
		    	anser_value=((2*temp)+Math.exp(start_point)+Math.exp(end_point))*h/2;
	...
	public static void Simpson(double start_point,double end_point){
		...
		for(int m=1;;m++){
		    ...
		    h=(end_point-start_point)/n;
		    for(int j=1;j<=(n/2);j++){
		    	temp_mid+=Math.exp(start_point+((2*j-1)*h));
		    }	    
		    for(int j=1;j<=(n/2)-1;j++){
		    	temp_tra+=Math.exp(start_point+(2*j*h));
		    }	     
		    //must add abs
		    if(Math.abs((Math.exp(start_point)+(4*temp_mid)+(2*temp_tra)+Math.exp(end_point))*h/3-anser_value)<0.000001 && m!=1){
		    	anser_m=m;
		    	anser_value=(Math.exp(start_point)+(4*temp_mid)+(2*temp_tra)+Math.exp(end_point))*h/3;
